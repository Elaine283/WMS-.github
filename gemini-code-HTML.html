...
<head>
  ...
  <style>
    /* ── Design tokens：WMS 溫柔零售選物 · 蜜桃櫻花焦糖色系 ── */
    :root {
      --bg-base: #FDF6EE;        /* 燕麥燕麥/暖白基底 */
      --bg-surface: #FAF0E6;     /* 牛奶 */
      --bg-elevated: #FAF0E6;   
      --bg-hover: #FDDAB4;       /* 桃子（作為懸停高亮） */
      
      --border: rgba(100, 65, 50, 0.08);
      --border-strong: rgba(100, 65, 50, 0.16);
      
      --text: #3D2B1F;           /* 濃咖啡（深主字體色） */
      --text-muted: #5C4033;     /* 復古深褐（次要字體色） */
      --text-faint: #C8956C;     /* 焦糖（輔助/註解/弱提示） */
      
      --accent: #E8A4A8;         /* 玫瑰 */
      --accent-hover: #F0A8B5;   /* 櫻花深 */
      --accent-muted: #F9C8D0;   /* 櫻花 */
      --success: #16C57A;        /* 精準對齊的原生翠綠 */
      --warning: #F8C59A;        /* 桃深 */
      ...
      --glass-bg: rgba(254, 246, 238, 0.75);
      --glass-border: rgba(232, 164, 168, 0.2);
    }
    ...
    *, *::before, *::after {
    ...
    .nav-link:hover {
      background: rgba(253, 218, 180, 0.4);
      color: var(--text);
    }
    ...
    .card {
      ...
      border-radius: var(--radius);
      padding: 1.5rem;
      box-shadow: 0 4px 20px rgba(100, 65, 50, 0.02);
    }
    ...
    .kpi__delta--up, .kpi__delta--down-good {
      color: var(--success);
      font-weight: 600;
    }
    ...
    .timeline__status--done {
      background: rgba(22, 197, 122, 0.1);
      color: var(--success);
    }
    ...
    .timeline__status--pending {
      background: rgba(100, 65, 50, 0.05);
      color: var(--text-faint);
    }
    ...
    .pain-quote {
      ...
      background: rgba(253, 218, 180, 0.15);
      font-style: italic;
    }
    ...
    @media (max-width: 768px) {
      ...
      .grid-2, .grid-kpi { grid-template-columns: 1fr; }
    }
  </style>
</head>
<body>
    ...
    <div class="main">
      ...
      <div class="content">
        ...
        <section id="analytics" class="section">
          ...
          <p class="section__lead">以下數據與圖表經由專案專屬演算法演算得出，由 `project.json` 數據檔案驅動，前端即時解構動態圖形。</p>
        ...
        <section id="future" class="section">
          ...
          <p class="section__lead">本研究專題不僅建立了完善的供應鏈數學模型，在系統發布、自動部署（CI/CD）也採用現代化最高規格。</p>
...
env:
  FORCE_JAVASCRIPT_ACTIONS_TO_NODE24: true</pre>
              <p>此配置能令整個靜態簡報系統即便在 Node.js 20 完全移除執行器後，依然能夠藉由最新的 Node.js 24 引擎高速、順暢且無中斷地編譯上線。 </p>
            </div>
            ...
            <div class="card prose">
              ...
              <ul>
                ...
                <li><strong>IoT 自動化無縫結合：</strong> 架構具備高擴充性，未來可向電子標籤、AMR 移動機器人或自動化立體倉儲 (AS/RS) 進行串接。</li>
              </ul>
            </div>
          </div>
        </section>
  ...
  <script>
    // 專案設定數據模擬（解構自 project.json，確保數據完美連動）
    const projectData = {
      "meta": {
        "title": "倉儲管理系統 (WMS) 與庫存最佳化",
        "subtitle": "結合零售電商倉儲實務與庫存最佳化 · 專題研究報告",
        "author": "陳玉鳳、洪依辰",
        "advisor": "褚文明",
        "department": "勤益科大 行銷與流通管理系所",
        "year": "2025–2026",
        "version": "1.3.0"
      },
      "kpis": [
        { "label": "庫存準確率", "value": "98.6%", "delta": "+2.4%", "trend": "up" },
        { "label": "庫存週轉率", "value": "8.4", "delta": "+12% 次/月", "trend": "up" },
        { "label": "倉儲利用率", "value": "87%", "delta": "+5%", "trend": "up" },
        { "label": "缺貨率", "value": "2.1%", "delta": "-0.8%", "trend": "down-good" },
        { "label": "訂單準時率", "value": "98.6%", "delta": "+2.3%", "trend": "up" },
        { "label": "揀貨效率", "value": "+35%", "delta": "vs 導入前", "trend": "up" },
        { "label": "庫存持有成本", "value": "-28%", "delta": "EOQ 優化", "trend": "down-good" },
        { "label": "入出庫處理時間", "value": "-42%", "delta": "平均 4.2h", "trend": "down-good" }
      ],
      "charts": {
        "turnoverTrend": {
          "labels": ["11月", "12月", "1月", "2月", "3月", "4月"],
          "datasets": [
            { "label": "庫存週轉率（次/月）", "data": [6.2, 6.8, 7.1, 7.5, 8.0, 8.4] },
            { "label": "導入前基準", "data": [5.4, 5.5, 5.6, 5.7, 5.8, 5.9] }
          ]
        },
        "inboundOutbound": {
          "labels": ["11月", "12月", "1月", "2月", "3月", "4月"],
          "datasets": [
            { "label": "入庫（千件）", "data": [42, 55, 48, 61, 57, 64] },
            { "label": "出庫（千件）", "data": [38, 50, 45, 58, 53, 62] }
          ]
        },
        "inventoryComparison": {
          "labels": ["入庫", "出庫", "盤點差異", "退貨", "調撥"],
          "data": [6400, 6200, 128, 286, 412]
        },
        "abcDistribution": {
          "labels": ["A 類（80% 金額）", "B 類（15%）", "C 類（5%）"],
          "data": [80, 15, 5]
        },
        "warehouseUtilization": {
          "labels": ["A 倉電子", "B 倉美妝", "C 倉食品", "D 倉服飾", "E 倉雜貨"],
          "data": [92, 78, 58, 85, 71]
        },
        "wmsImpact": {
          "labels": ["作業錯誤率", "出貨時間", "庫存準確率", "訂單準時率"],
          "before": [8.5, 100, 92, 94],
          "after": [2.1, 58, 98.6, 98.6]
        }
      },
      "timeline": [
        { "phase": "需求訪談與 WMS 文獻", "period": "Week 1–4", "status": "done" },
        { "phase": "庫存模型與系統設計", "period": "Week 5–8", "status": "done" },
        { "phase": "WMS 原型與 Dashboard", "period": "Week 9–14", "status": "done" },
        { "phase": "模擬測試與 KPI 驗證", "period": "Week 15–18", "status": "active" },
        { "phase": "專題報告撰寫", "period": "Week 19–20", "status": "pending" }
      ]
    };
    ...
    // 💡 適配溫柔零售選物風的 Chart.js 調色盤
    const CHART_COLORS = {
      primary: "#E8A4A8",       // 玫瑰粉
      primaryDim: "rgba(232, 164, 168, 0.7)", 
      secondary: "#C8956C",     // 焦糖木質
      secondaryLight: "rgba(200, 149, 108, 0.3)",
      muted: "#5C4033",         // 復古褐
      grid: "rgba(100, 65, 50, 0.08)", // 溫和拿鐵線條
      text: "#3D2B1F",          // 深黑咖啡
    };
      ...
      plugins: {
        legend: {
          labels: {
            ...
            font: { family: "'IBM Plex Sans', 'Noto Sans TC', sans-serif", size: 11, weight: '500' },
            boxWidth: 12,
            padding: 14,
          },
        },
        tooltip: {
          backgroundColor: "#FAF0E6", // 牛奶底
          titleColor: "#3D2B1F",
          bodyColor: "#5C4033",
          borderColor: "#E8A4A8",
          borderWidth: 1,
    ...
    function renderNav() {
      ...
      const groups = [
        ...
        { title: "WMS 理論", ids: ["background", "comparison", "objectives", "wms-modules", "inventory-optimization", "ecommerce", "literature"] },
        { title: "研究實作", ids: ["methodology", "architecture", "analytics"] },
        { title: "總結", ids: ["future"] }
      ];
    ...
    function applyMeta(data) {
      ...
      if (kpiGrid && kpis) {
        kpiGrid.innerHTML = kpis
          .map((k) => {
            const deltaClass = (k.trend === "up" || k.trend === "down-good") ? "kpi__delta--up" : "";
            return `
          <div class="card kpi">
    ...
    function initCharts(data) {
      const { turnoverTrend, inboundOutbound, inventoryComparison, abcDistribution, warehouseUtilization, wmsImpact } = data.charts;
        ...
        new Chart(lineCtx, {
          ...
          data: {
            ...
            datasets: turnoverTrend.datasets.map((ds, i) => ({
              ...
              backgroundColor: i === 0 ? "rgba(232, 164, 168, 0.15)" : "transparent",
              borderWidth: 2.5,
              borderDash: i === 0 ? [] : [4, 4],
              pointRadius: 4,
              pointBackgroundColor: i === 0 ? CHART_COLORS.primary : CHART_COLORS.secondary,
              tension: 0.35,
              fill: i === 0,
            })),
          },
          options: { ...chartDefaults, scales: scaleOptions() },
        });
      }
        ...
        new Chart(barCtx, {
          ...
          data: {
            ...
            datasets: [{
              ...
              backgroundColor: [CHART_COLORS.primaryDim, "rgba(200, 149, 108, 0.75)", "rgba(248, 197, 154, 0.8)", "rgba(92, 64, 51, 0.4)", "rgba(240, 168, 181, 0.6)"],
              borderRadius: 4
            }]
          },
          options: { ...chartDefaults, scales: scaleOptions() },
        ...
        new Chart(donutCtx, {
          ...
          data: {
            ...
            datasets: [{
              ...
              backgroundColor: [CHART_COLORS.primary, "#F8C59A", "#C8956C"],
              borderWidth: 2,
              borderColor: "#FAF0E6"
            }]
          },
      ...
      if (impactCtx && wmsImpact) {
        new Chart(impactCtx, {
          ...
          data: {
            ...
            datasets: [
              { label: "導入前基準", data: wmsImpact.before, backgroundColor: "rgba(92, 64, 51, 0.3)", borderRadius: 4 },
              { label: "導入後效益", data: wmsImpact.after, backgroundColor: CHART_COLORS.primaryDim, borderRadius: 4 }
            ]
          },
          options: { ...chartDefaults, scales: scaleOptions() },
        });
        ...
        new Chart(utilCtx, {
          ...
          data: {
            ...
            datasets: [{
              ...
              backgroundColor: warehouseUtilization.data.map((v) => v >= 85 ? CHART_COLORS.primary : "rgba(200, 149, 108, 0.6)"),
              borderRadius: 4
            }]
          },
          options: {
            ...chartDefaults,
            scales: { ...scaleOptions(), y: { ...scaleOptions().y, max: 100 } }
          },
        });
      }
    }
    ...
    function main() {
      renderNav();
      initSidebar();
      applyMeta(projectData);
      initCharts(projectData);
      initScrollSpy();
